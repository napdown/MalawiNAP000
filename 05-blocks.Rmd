...
# set py version to use
#reticulate::use_python("C:/Users/Makabe/AppData/Local/r-miniconda/envs/rgee/python.exe")
...
7.	Address capacity gaps on investment in skills and capabilities for negotiations, mechanisms for reducing emissions while supporting prudent environmental management and sustainable economic growth.   
...
12.	Integrate population issues into climate change management in the development agenda through an integrated approach which would reduce poverty, protect natural resources and reduce inequality.  
13.	Incorporate HIV and AIDS as well as gender considerations in all climate change interventions including adaptation, mitigation, capacity building and technology development and transfer.  
...
t6<-bold(t5,j=1)
t6<-set_caption(t6, "National/sectoral policies, strategies and plans relevant for adaptation")
t6
```
...
envt<-readxl::read_excel("Tables_from_NAP.xlsx", 
    sheet = "envt_policies")
t1<-flextable(envt,col_keys = names(envt),
  cwidth = 3,
  cheight = 0.01,
  defaults = list(),
   theme_booktabs())
...
t5<-bold(t4,part='header')
t6<-bold(t5,j=1)
t6<-set_caption(t6, "General environmental laws and their policy relevance for adaptation")
t6
```
...
![Table 3: Evolution of the national climate change agenda in Malawi: policies, programmes, institutions and linkage to political leadership (Cacho et al., 2010)](images/cc_policies_evolution.png)
