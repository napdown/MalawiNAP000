...
t6<-set_caption(t5, "Examples of notable past extreme climate events, impacts and impacted systems in Malawi")
t6
```


![Figure 2: Common shocks experienced from flooding in 2016-2017 Mzuzu town according to the IHS3 (Kita 2017)](images/mzuzu_shocks.png)
...
![Figure 3. Trends in people in need of food assistance](images/food_assistance_trends.png)
...
![](images/gdp_per_worker.png)
_Figure 4: (a) Malawi's per worker agricultural value-added and national per capita GDP adjusted for constant 2005 prices for 1980-2008 (World Bank 2015); and (b) per capita maize production and price for the same period (Chirwa et al. 2006; FAOSTAT 201)_
...
```{r}
exposure<-readxl::read_excel("Tables_from_NAP.xlsx", 
    sheet = "livelihood_exposure")
t1<-flextable(exposure,col_keys = names(exposure),
  cwidth = 3,
  cheight = 0.01,
  defaults = list(),
   theme_booktabs())
...
t5<-bold(t4,part='header')
t6<-bold(t5,j=1)
t6<-set_caption(t6, "Exposure of household livelihoods to climatic risks in the past ten years in Malawi (N=1582) (Abdi et al., 2020)")
t6
```

_Source:  CIMMYT-led project on Sustainable Intensification of Maize and Legume Cropping Systems for Food Security in Eastern and Southern Africa (SIMLESA) (Abdi et al., 2020)_
...
![Figure 5: Historic climate variability in Malawi.](images/cc_historic.png)

_(Source: The International Resources Institute for Climate and Society at Columbia University, derived from the Climate Research Unit at the University of East Anglia, the United Kingdom)._ 
_Note: Yellow-red shading (drought) shows the country's percentage that would experience lower than normal rainfall (to different degrees). Blue shading (floods) indicates the country's percentage that would experience higher than normal rainfall linked to floods._



![Figure 6: Seasonal rainfall time series Zomba.](images/pr_ts.png)
_Source: (Jørstad & Webersik, 2016)_

__Temperature and Precipitation Anomalies in Malawi from 1950 to present (based on climatological period 1981-2010)__
...
![Figure 7: Changes in heat extremes in the 2030s and 2040s (Source: World Bank. 2017(c). Multi-Sectoral Investment Plan for Climate and Disaster Risk Management in Malawi)](images/heat_extremes.png)
...
plot(prvar585,sub=paste(names(prvar585)), legend=T)
plot(malawisf$geometry, add=T)
...
![Figure 8: Time series of mean annual temperature (°C) for 34 CMIP models (Source: UMFULA. 2017. Malawi Country Climate Brief: Future Climate Change Projections for Malawi). Note: CMIP = Climate Model Intercomparison Project](images/tavg_cmip_all.png)
...
![Table 6: Relative yield projections in 2050 for main crops grown by smallholders in Malawi expressed as the ration of yields under each RCP to yields under no climate change reference scenario with CO2 fertilizer (± SD)](images/yield_proj_maincrops.png)
 

 
![Figure 9: Average changes in the Lilongwe District's mid-and late century maize yields(%).Compared to the baseline period (1971-2000), the average maize yield reduction ranges for the 2050s and 2090s are 7%-14% and 13% -33%, respectively.Source: Msowoya et al., 2016](images/maize_yields_lilongwe.png)
...
t4<-border_inner(t3,part = 'all', border = NULL)
t5<-bold(t4,part='header')
t6<-set_caption(t5, "Sub-national climate projection data for 2020 – 2099 period")
t6
```
...
#NV.AGR.PCAP.KD.ZG -2012
#NV.AGR.TOTL.ZG -2012
#NV.AGR.TOTL.ZS 2020

# Agriculture, forestry, and fishing, value added (% of GDP)
agrVA<-wb_data("NV.AGR.TOTL.ZS", country = "Malawi",start_date = 1961,end_date = 2020, return_wide = FALSE)%>%group_by(date)%>%summarise(value=sum(value))
colnames(agrVA)[colnames(agrVA)=="value"]<-'Agriculture contrubution to GDP'

# GDP growth (annual %)
gdp<-wb_data("NY.GDP.MKTP.KD.ZG", country = "Malawi",start_date = 1961,end_date = 2020, return_wide = FALSE)%>%group_by(date)%>%summarise(value=sum(value))
colnames(gdp)[colnames(gdp)=="value"]<-'GDP Growth'

# temperature
temp_ann<-tmpdf%>%dplyr::filter(Year>=1961)%>%group_by(Year)%>%summarise('tmean'=mean(tmean))
pr_ann<-prdf%>%dplyr::filter(Year>=1961)%>% group_by(Year)%>%
  summarise(across(contains("pr"), ~mean(pr))) 

df<-data.frame(agrVA,gdp)
df<-df[,-3]
#df %>% pivot_longer(-date) %>% ggplot(aes(x = date, y = value, colour = name)) +
  #geom_path(size = 0.8) +labs(x='Year', y='Value (%)')+
  #ggtitle('Annual GDP Growth and Agriculture, forestry, and fishing, value added (% of GDP)')+
  #theme(legend.title=element_blank(), legend.position = 'top')


p1<-plot_ly(data=df,x=~date)%>%
  add_lines(y=~Agriculture.contrubution.to.GDP, name='Agriculture contribution to GDP')%>%
  add_lines(y=~GDP.Growth,name='GDP Growth')%>%
  plotly::layout(legend=list(x=0.7,y=1),xaxis=list(title="Year"),yaxis=list(title="Value (%)",side='right'))


df2<-data.frame(pr_ann,temp_ann)
df2<-df2[,-3]
...
p2<-plotly::plot_ly(data= df2, x=~Year, y=~pr)%>%
  add_lines(y= ~pr,name = 'Precipitation')%>%
  plotly::add_lines(y= ~tmean, name= 'Temperature', yaxis='y2')%>%
  plotly::layout(legend=list(orientation='h', y=-0.13,x=0.2), yaxis=list(title='Precipitation (mm)',showticklables=F), title='GDP and Climate trends (1991-2020)', yaxis2=ty, xaxis=list(tickangle=-90,dtick=20))

subplot(p2,p1, nrows = 1, margin = 0.04, shareX = TRUE, titleY = TRUE)%>%
  layout(legend = list(x = 0.01, y = -0.19))%>%
...
t5<-bold(t4,part='header')
t6<-bold(t5, j=1)
t6<-set_caption(t6, "Observed climate impacts at Country Level Impact of Climate Change (CLICC) in Malawi. ")
t6
...
proj_impacts<-readxl::read_excel("Tables_from_NAP.xlsx", 
    sheet = "proj_impacts_agrifood")
t1<-flextable(proj_impacts,col_keys = names(proj_impacts),
  cwidth = 3,
  cheight = 0.01,
  defaults = list(),
   theme_booktabs())
...
t5<-bold(t4,part='header')
t6<-bold(t5, j=1)
t6<-set_caption(t6, "Projected climate change impacts on the agri-food system. With medium rating based on medium quality data, agricultural yield will exhibit declining trends.")
t6

```

__Source:__ CLICC Phase 2 Project (2019).
...
```{r}
library(readr)
crop_liv<-read.csv("C:/Users/Makabe/opennaps/MalawiNAP000/Production_Crops_Livestock_E_All_Data_(Normalized).csv", header=T, sep = ',', stringsAsFactors = T)
maizeYld<-crop_liv%>% dplyr::filter(Area=='Malawi'|Item=="Maize"|Element=='Yield')%>%na.omit()%>%group_by(Year)%>%
  summarise(Value=sum(Value)/1000)

# convert hg/ha to mt/ha
maizeYld$Value<-maizeYld$Value*0.0001
  
mplot<-ggplot2::ggplot(maizeYld,aes(x=Year, y=Value))+
...
mycrops<-crop_liv%>%dplyr::filter(Area=="Malawi")%>%dplyr::filter(Element=="Yield")%>%dplyr::filter(Item=="Tobacco, unmanufactured"|Item=="Tea"|Item=="Sugar cane")%>%group_by(Item,Year)%>%
  summarise(Value=sum(Value)/1000)

# convert hg/ha to mt/ha
#mycrops$Value<-mycrops$Value*0.0001
  
mplot<-ggplot2::ggplot(mycrops,aes(x=Year, y=Value, col=Item))+
  ggtitle('Annual Yield Trend for selected cash crops in Malawi 1961-2020')+
  xlab('Year')+ylab('Yield (\'000 hg/ha)')+
  geom_line()+ 
  #geom_point()+
  theme_grey()
plotly::ggplotly(mplot)
...
```{r}
livestock<-crop_liv%>%dplyr::filter(Area=="Malawi")%>%dplyr::filter(Item=="Cattle"|Item=="Goats")%>%group_by(Item,Year)%>%summarise(Value=sum(Value)/1000)

# convert hg/ha to mt/ha
#mycrops$value<-mycrops$value*0.0001
  
mplot<-ggplot2::ggplot(livestock,aes(x=Year, y=Value, col=Item))+
  ggtitle('Stocking trends for selected livestock in Malawi 1961-2020')+
  xlab('Year')+ylab('Heads (\'000)')+
  geom_line()+ 
  #geom_point()+
  theme_grey()
plotly::ggplotly(mplot)
...
__The Country Vision on Trade.__
...
![Figure 10: Value-added breakdown of the Malawian GDP between 1990 and 2017](images/GDP_2017.png)
_Source:  UNCTAD secretariat calculations based on World Bank World Development Indicators data_


![Figure 11: Food trade calculations for exports and imports between 1995 and 2017. Source: UNCTAD secretariat calculations based on World Bank World Development Indicators data](images/food_trade.png) 
```{r, eval=FALSE}

data_folder <- "FAOSTATs"
dir.create(data_folder)
alldata<-FAOsearch()
#perworkergdp<-get_faostat_bulk(code = "OE", data_folder = data_folder)
foodbal<-get_faostat_bulk(code='FBS', data_folder = data_folder)
...
eco<-readxl::read_excel("Tables_from_NAP.xlsx", 
    sheet = "ecosystems")
t1<-flextable(eco,col_keys = names(eco),
  cwidth = 3,
  cheight = 0.01,
  defaults = list(),
   theme_booktabs())
...
t4<-border_inner(t3,part = 'all', border = NULL)
t5<-bold(t4,part='header')
t6<-set_caption(t5, "Selected ecological systems of special significance in Malawi")
t6
...
fish<-readxl::read_excel("Tables_from_NAP.xlsx", 
    sheet = "fish_prod")
t1<-flextable(fish,col_keys = names(fish),
  cwidth = 3,
  cheight = 0.01,
  defaults = list(),
   theme_booktabs())
...
t4<-border_inner(t3,part = 'all', border = NULL)
t5<-bold(t4,part='header')
t6<-set_caption(t5, "Trends in Malawi's Annual Fish Production and Growth for Capture Fisheries And Aquaculture")
t6

```

_Source: Department of Fisheries _
...
![Figure 12: Potential sites of aquaculture investment in Malawi. source:](images/aquacilture_invest.png)  
...
dir.create(data_folder)
#alldata<-FAOsearch()
fish_inv <- get_faostat_bulk(code = "CISP", data_folder = data_folder)
saveRDS(fish_inv, "FAOSTATs/fish_inv_e_all_data.rds")
fish_inv <- readRDS("FAOSTATs/fish_inv_e_all_data.rds")

```
```{r}
fish_usd<-fish_inv%>%filter(area=='Malawi', unit=='millions')
fish_pc<-fish_inv%>%filter(area=='Malawi', unit=="%")


plot_ly(data= fish_usd, x=~year, color = ~item)%>%
  add_lines(y=~value)%>%
  plotly::layout(legend=list(orientation='h', y=-0.13,x=0.7), yaxis=list(title='Amount (\'000 USD)',showticklables=F), title='Investment Trends in Fishing Industry in Malawi (1991-2020)', yaxis2=ty, xaxis=list(tickangle=-90,dtick=20))
...
water<-readxl::read_excel("Tables_from_NAP.xlsx", 
    sheet = "impacts_water")
t1<-flextable(water,col_keys = names(water),
  cwidth = 3,
  cheight = 0.01,
  defaults = list(),
   theme_booktabs())
...
t5<-bold(t4,part='header')
t6<-bold(t5,j=1)
t6<-set_caption(t6, "Observed impacts of climate change on the water system in Malawi. Water supply, water treatment, water collection and surface water management are all affected by the impacts of climate change.")
t6
...
__Subsystems: Rural water supply; Urban water supply system – Lilongwe City; Urban water supply system – Mzuzu City; Urban water supply system – Blantyre City__
...
__Subsystems: Hydroelectricity generation system; Electricity distribution infrastructure__
...
__Subsystems: Urban waste management system - Landfills; Medical waste management; Sewage waste management systems__
...
![Figure 13: Malawi systems August 2020 LEG map.](images/LEG_map.jpg)
